...
// Peaked Dia glow: stacked smooth bezier peaks (light front → dark back), each blurred
// into one soft mountain of colour rising from the bottom.
...
export function PeakedGradient({
	...
	const blurId = `peak-blur-${useId().replace(/:/g, "")}-${replayKey}`;
	...
	return (
		...
		>
			...
			>
				<title>Footer peaked gradient glow</title>
				<defs>
				...
				<g filter={`url(#${blurId})`}>
					{layers.map((layer, i) => (
						<path key={i} d={layer.d} fill={layer.color} />
					))}
				</g>
			</svg>
		</div>
